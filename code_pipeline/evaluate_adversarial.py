...
def code_generate(args, workdir: PathLike, model: DecoderBase, id_range=None):
    with Progress(
        ...
        if args.dataset == "humaneval":
            dataset = {}
            path = "/share5/ru.wang/code/Qwen2.5-Coder/qwencoder-eval/instruct/eval_plus/result/HumanEval"
            file_list = os.listdir(path)
            for file in file_list:
                with open(os.path.join(path, file), "r") as f:
                    data = json.load(f)
                    dataset[file.replace("_", "/")] = {"prompt": data["problem_description"], "contract": ""}
        elif args.dataset == "mbpp":
            dataset = {}
            path = "/share5/ru.wang/code/Qwen2.5-Coder/qwencoder-eval/instruct/eval_plus/result/mbpp"
            file_list = os.listdir(path)
            for file in file_list:
                with open(os.path.join(path, file), "r") as f:
                    data = json.load(f)
        ...
        for task_id, task in p.track(dataset.items()):
            ...
            p_name = task_id.replace("/", "_").replace(".jsonl", "")
