...
def evaluate_one_file(sample: dict, data_name: str, prompt_type: str, execute: bool = False) -> bool:
    # Parse ground truth
    _, gt = parse_ground_truth(sample, data_name)
    
    # Get prediction
    if not sample.get('pred'):
        return False
    pred = sample['pred'][-1] # Take last prediction
    
    # Compare prediction with ground truth
    try:
        # Use sample's idx if available, otherwise default to 0
...
def evaluate_without_gtcot(data_name, prompt_type, samples: list=None, file_path: str=None, max_num_samples=None, execute=False):
    ...
    for sample in samples:
        sample['gt'] = parse_ground_truth_without_gtcot(sample, data_name)
